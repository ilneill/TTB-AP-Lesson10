...
# Work out the Arduino bgrLEDs command action.
def bgrLEDsAction(sensorValue = "nan", bgThreshold = 5, grThreshold = 30, hysteresis = 0.5):
    ...
    if sensorValue != "nan":
        ...
        if bgThreshold <= sensorValue <= grThreshold: # Within the thresholds for green.
            arduinoAction = 2                         # Bit 1 set to 1.
        if sensorValue > grThreshold:                 # Over the green-red threshold.
...
class meterType1:
    def __init__(self, mt1Pos = vector(0, 0, 0), mt1Color = color.red, mt1ScaleMin = 0, mt1ScaleMax = 5, mt1Label = "", mt1Units = ""):
        ...
        # Add a 30%/70% band indicator bgrColorLED.
        self.voltageBGRLED  = bgrColorLED(vector(0.75, -0.425, 0.05) + self.mt1Pos, self.mt1ScaleRange * 0.3, self.mt1ScaleRange * 0.7)
        # Corner screws.
    ...
    def update(self, mt1Value = "nan", mt1RawValue = "-1"):
        if mt1Value != "nan":
            ...
            # Update the bgrLED.
            self.voltageBGRLED.update(self.mt1Value)
        else:
...
class bgrLEDBank():
    def __init__(self, bgrLEDBankPos = vector(0, 0, 0), bgrInARow = True, bgThreshold = 5, grThreshold = 30, hysteresis = 0.5):
        ...
        if self.bgrInARow:  # Draw the LEDs horizontally.
...
# A 3 discrete BGR color LED.
class bgrColorLED():
    def __init__(self, bgrColorLEDPos = vector(0, 0, 0), bgThreshold = 1.5, grThreshold = 3.5, hysteresis = 0.5, offColor = color.gray(0.5)):
        self.bgrColorLEDPos = bgrColorLEDPos
        self.offColor = offColor
        self.bgThreshold = bgThreshold
        self.grThreshold = grThreshold
        self.hysteresis = hysteresis # TODO: Adjust thresholds to stabilise the LEDs if a reading is jittering around a boundary.
        self.ledDome = sphere(color = self.offColor, opacity = 1, radius = 0.1, pos = vector(0, 0, 0.15) + self.bgrColorLEDPos)
        self.ledBody = cylinder(color = self.offColor, opacity = 1, pos = vector(0, 0, 0.15) + self.bgrColorLEDPos, axis = vector(0, 0, -0.1), radius = 0.1)
        self.ledBase = cylinder(color = self.offColor, opacity = 1, pos = vector(0, 0, 0.05) + self.bgrColorLEDPos, axis = vector(0, 0, -0.05), radius = 0.125)
        cylinder(color = color.white, opacity = 1, pos = vector(-0.05, 0, 0) + self.bgrColorLEDPos, axis = vector(0, 0, -0.2), radius = 0.01)
        cylinder(color = color.white, opacity = 1, pos = vector(0.05, 0, 0) + self.bgrColorLEDPos, axis = vector(0, 0, -0.25), radius = 0.01)
    def update(self, sensorValue = "nan"):
        # If we have valid data.
        if sensorValue != "nan":
            self.sensorValue = sensorValue
...
class triColorLED():
    def __init__(self, triColorLEDPos = vector(0, 0, 0), offColorR = 127, offColorG = 127, offColorB = 127):
        self.triColorLEDPos = triColorLEDPos
        ...
        self.ledDome = sphere(color = self.offColor, opacity = 1, radius = 0.1, pos = vector(0, 0, 0.15) + self.triColorLEDPos)
        self.ledBody = cylinder(color = self.offColor, opacity = 1, pos = vector(0, 0, 0.15) + self.triColorLEDPos, axis = vector(0, 0, -0.1), radius = 0.1)
        self.ledBase = cylinder(color = self.offColor, opacity = 1, pos = vector(0, 0, 0.05) + self.triColorLEDPos, axis = vector(0, 0, -0.05), radius = 0.125)
        cylinder(color = color.white, opacity = 1, pos = vector(-0.05, 0, 0) + self.triColorLEDPos, axis = vector(0, 0, -0.2), radius = 0.01)
        cylinder(color = color.white, opacity = 1, pos = vector(0.05, 0, 0) + self.triColorLEDPos, axis = vector(0, 0, -0.25), radius = 0.01)
    def update(self, triColorLEDR = "default", triColorLEDG = "default", triColorLEDB = "default"):
        if triColorLEDR == "default":
            self.colorR = self.offColorR
        else:
            self.colorR = triColorLEDR / 255
        if triColorLEDG == "default":
            self.colorG = self.offColorG
        else:
            self.colorG = triColorLEDG / 255
        if triColorLEDB == "default":
            self.colorB = self.offColorB
        else:
            self.colorB = triColorLEDB / 255
...
while True:
    ...
    if not pseudoDataMode: # We are not virtual meter testing with pseudo random data.
        if serialOK:
            bgrLEDsArduinoUpdate = bgrLEDsAction(tDHT11)
            # Only send an update to the Arduino if the bgrLEDs status has changed from the last time.
            if bgrLEDsArduinoUpdate != bgrLEDsArduino:
                # Construct the command to send to the Arduino.
                arduinoCmd = "bgrLEDs=%d\n" % bgrLEDsArduinoUpdate
                # Encode and send the command to the Arduino. 
                arduinoDataStream.write(arduinoCmd.encode())
                # Update the current Arduino bgrLEDs status.
                bgrLEDsArduino = bgrLEDsArduinoUpdate
